...
An enterprise web services platform — offering website builders, domains, hosting, email, and marketing services for small and medium businesses — was struggling to convert website visitors. Many arrived intending to purchase, but a significant share left without buying, with confusion over the product offering as the leading cause. Users needed the platform to help them identify the right package.
...
**Constraints:** cross-functional impact (the CMS, storefront, PMM, analytics, creative, SEO teams) — any navigation change required coordination across at least 6 teams.
...
Both "explorers" (browsing broadly) and "seekers" (looking for known terms) oriented naturally to the same four-category top-level structure — grouped by the outcome they were after (establishing an online presence, building an online store, adding specific features) plus pricing, which was consistently sought after an initial page scan. The current structure organized by product line (websites, marketing, domains, hosting, security, email) didn't map to how visitors described their own needs.
...
- Interactive Figma prototype (wireframes + interactive walkthrough)
...
- Participants consistently praised the prototype for surfacing pricing on the same page as the relevant products, for a clean and uncluttered interface, and for bullet-point product descriptions that were easier to digest than dense copy — several described it as the best experience of the sites they compared
...
2. **Restructure primary nav** into the four validated top-level groupings
...
- Post-launch click-heatmap analysis to confirm the four-category structure holds under real-world traffic patterns
...
*Prototype visuals omitted for client confidentiality.*

The interactive prototype covered the home page (four-category primary navigation with a domain search bar), a products mega menu (each item with a name and contextual description), a profile-based category landing page with secondary anchor navigation, and an inline pricing comparison page directly accessible from the primary nav.
