...
Facilitated the Design Sprint with the manufacturer's commercial team and key account managers (KAMs), and designed the validation artifacts — a landing page presenting the program and a companion app addressing the program's highest-friction moments.

**Secondary: UX Researcher (validation interviews).**
Built the persona and screening criteria, ran the recruitment funnel, designed the four validation journeys, conducted the validation interviews with ISP owners, and synthesized findings into a validation report across two rounds.
...
**Four validation journeys.**
The program had to work across different paths to purchase, so validation was designed as four parallel scenarios:
- **J1 — Self-service landing page:** the ISP finds the program's landing page on their own and can request a consultant quote, a network-expansion pre-project, and credit analysis. Facilitators mediated to simulate solitary navigation.
- **J2 — KAM-assisted:** a key account manager phones the ISP, presents the program, sends the landing-page link, and accompanies the provider through it.
- **J3 — Factory visit for sale:** prospective buyers visit the factory; the validation criterion is the purchase-closing decision itself.
...
1. **Lead the program with its commercial model, not its hardware.** The validated message is financing + network valuation + training; the equipment is what that model delivers, not the headline.
2. **Design the digital path to hand off to a person before the financial ask.** Use the landing page and app to inform, qualify, and schedule — let a known account manager carry the financing conversation. Do not ask for financial data on a first-touch web form.
3. **Treat the factory visit as a core sales asset, not a perk.** It converts because it supplies the peer-and-trust signal the equipment lacks elsewhere. Standardize the route, protect the equipment-presentation time, and keep the accompanying group small enough to keep focus on the visitor.
...
**What worked well.**
Designing validation as four parallel journeys rather than one happy path was the right call. It exposed that the program had a path-to-purchase problem (the financial-data web form) that a single self-service test would have flattered. Running a *physical* validation round alongside the digital one was equally important — the factory open-house revealed a conversion mechanism that no prototype could have surfaced.

**The most useful finding ran against the brief.**
The engagement began as a way to sell more active equipment, and the most valuable insight was that the equipment was beside the point. ISPs don't buy boxes on features; they buy on peer reference and price, and on a commercial model that de-risks an expansion. Naming that early reframed the whole program — and is the kind of finding that only emerges when you validate with the actual decision-maker rather than the person who specs the hardware.

**What I'd do differently.**
The first landing-page concept asked for too much, too soon, from a conservative B2B buyer. I'd start the next iteration from the relationship — what a provider will share, and through which channel, at each stage of trust — and let the interface follow that, rather than designing the form first and discovering the trust mismatch in testing.

<!-- TODO: create assets/manufacturing-b2b-design-sprint-2019/ and add relevant visuals (landing-page + app prototype frames, journey map, open-house route) -->
